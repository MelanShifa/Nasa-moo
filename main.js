...
function createBox({ lat, lng, location, scale }) {
  ...
  box.population = scale
}

function createBoxes(countries) {
  countries.forEach((country) => {
    const scale = country.scale / 1000000000
    const lat = country.latitude
    const lng = country.longitude
    const zScale = 0.1 * scale

    const box = new THREE.Mesh(
      new THREE.BoxGeometry(
        Math.max(0.1, 0.1 * scale),
        Math.max(0.1, 0.1 * scale),
        Math.max(zScale, 0.2 * Math.random())
      ),
      new THREE.MeshBasicMaterial({
        color: '#3BF7FF',
        opacity: 0.4,
    ...
    box.country = country.location
    box.population = country.scale
  })
}
...
createBox({
  ...
  lat: 23.473146,
  lng: -19.0,
  sensorDescription:"Apollo PSE Alsep Seismometer",
  scale:"1.00881E10"
})
createBox({
  location: "MH1",
...
const populationEl = document.querySelector('#locationEl')
const populationValueEl = document.querySelector('#scaleValueEl')
...
function animate() {
  ...
  for (let i = 0; i < intersects.length; i++) {
    ...
    populationEl.innerHTML = box.location

    populationValueEl.innerHTML = box.scale
  }
